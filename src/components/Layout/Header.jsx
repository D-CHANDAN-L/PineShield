    ...
    <header className="border-b border-slate-200 dark:border-[#243044] bg-white/95 dark:bg-[#161D2B]/95 backdrop-blur-md sticky top-0 z-30 px-3 sm:px-6 py-2.5 transition-colors">
      <div className="max-w-7xl mx-auto flex items-center justify-between gap-2 sm:gap-4">
        ...
        <div className="flex items-center space-x-2 sm:space-x-2.5 flex-shrink-0">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-full bg-[#161D2B] hover:bg-[#1E293B] border border-slate-700 text-emerald-400 transition cursor-pointer"
            title="Configure Google Gemini API key"
          >
            <Sparkles className="w-3.5 h-3.5 text-emerald-400" />
            <span className="hidden sm:inline">
              {typeof window !== 'undefined' && localStorage.getItem('PINELABS_GEMINI_KEY') ? 'Gemini: Active' : 'Set Gemini Key'}
            </span>
            <span className="sm:hidden">
              {typeof window !== 'undefined' && localStorage.getItem('PINELABS_GEMINI_KEY') ? 'Active' : 'Key'}
            </span>
          </button>
      ...
      <div className="flex md:hidden items-center justify-between pt-2 gap-2 overflow-x-auto">
        ...
        <div className="flex sm:hidden items-center gap-1.5">
          <button
            onClick={() => setIsApiKeyModalOpen(true)}
            className="flex items-center gap-1 px-2 py-1 rounded-xl border border-slate-200 dark:border-slate-800 text-[10px] text-amber-500 font-semibold bg-slate-50 dark:bg-[#0B0F17]"
          >
